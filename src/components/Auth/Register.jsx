import { Box, Flex, Input, Button, Link, Text, For, Center, Grid } from "@chakra-ui/react"
...
const Register = () => {
    const { CreateEmailConfirmation } = useRegister()
    ...
    return (
        ...
        >
            ...
            >
                ...
                <Center>
                    <Flex>
                        <Grid templateColumns="repeat(2, 1fr)" gapX={"30px"}>
                            <For each={[
                                { id: 1, label: "Name", required: true, input: Input, labelJSON: "name" },
                                { id: 2, label: "Surname", required: false, input: Input, labelJSON: "surname" },
                                { id: 3, label: "Patronymic", required: false, input: Input, labelJSON: "patronymic" },
                            ]}>
                                {(item) =>
                                    <Field key={item.id} label={item.label} required={item.required} marginBottom={"20px"} width={"100%"}>
                                        <item.input _focus={{ border: "2px solid var(--input-focus-border)", outline: "none" }} onChange={(e) => handlingInput(e, item.labelJSON)} />
                                    </Field>
                                }
                            </For>
                            <For each={[
                                ...
                                {(item) =>
                                    <Field key={item.id} label={item.label} required marginBottom={"20px"} width={"100%"}>
                                        <item.input _focus={{ border: "2px solid var(--input-focus-border)", outline: "none" }} onChange={(e) => handlingInput(e, item.labelJSON)} />
                                    </Field>
                                }
                            </For>
                ...
                <Button
                    ...
                    onClick={() => {
                        CreateEmailConfirmation(dataEmail, dataRegister);
